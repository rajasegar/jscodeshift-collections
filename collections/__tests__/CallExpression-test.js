...
test('find all call expressions', () => {
  const nodes = [recast.parse('foo()', 'bar(a,b,c)')];
...
test('find all member expressions', () => {
  const nodes = [recast.parse('foo.bar()')];

  const declarators =
    Collection.fromNodes(nodes).findCallExpressions('foo.bar');
  expect(declarators.getTypes()).toContain('CallExpression');
});

test('find all nested member expressions', () => {
  const nodes = [recast.parse('foo.bar.baz()')];

  const declarators =
    Collection.fromNodes(nodes).findCallExpressions('foo.bar.baz');
  expect(declarators.getTypes()).toContain('CallExpression');
});
test('rename call expressions', () => {
  const nodes = [recast.parse('foo()', 'bar(a,b,c)')];

  Collection.fromNodes(nodes).findCallExpressions('foo').renameTo('xyz');
...
test('rename member expressions', () => {
  const nodes = [recast.parse('foo()')];

  Collection.fromNodes(nodes).findCallExpressions('foo').renameTo('foo.bar');
...
test('remove params', () => {
  const nodes = [recast.parse('bar(a,b,c)')];

  Collection.fromNodes(nodes).findCallExpressions('bar').removeParam('a');
...
test('add params', () => {
  ...
  Collection.fromNodes(nodes).findCallExpressions('bar').addParam('d');
