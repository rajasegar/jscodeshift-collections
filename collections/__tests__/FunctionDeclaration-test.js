...
test('find all function declarations', () => {
  const nodes = [recast.parse('function foo() {}', 'var a = 1')];
...
test('rename function declarations', () => {
  const nodes = [recast.parse('function foo() {}', 'function bar(a,b,c) {}')];
  Collection.fromNodes(nodes).findFunctionDeclarations('foo').renameTo('xyz');

  const declarators =
    Collection.fromNodes(nodes).findFunctionDeclarations('xyz');
  expect(declarators.length).toBe(1);
});

test('remove params', () => {
  const nodes = [recast.parse('function foo() {}', 'function bar(a,b,c) {}')];
  Collection.fromNodes(nodes).findFunctionDeclarations('bar').removeParam('a');

  const declarators = Collection.fromNodes(nodes).find(types.Identifier, {
    name: 'a',
  });
...
test('add params', () => {
  const nodes = [recast.parse('function bar(a,b,c) {}')];
  Collection.fromNodes(nodes).findFunctionDeclarations('bar').addParam('d');
